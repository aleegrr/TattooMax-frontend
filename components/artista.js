...
const Artista = ({ name, id }) => {
  ...
  const fetchArtist = async () => {
    const data = await fetch(
      "https://tattoomax-backend-48q8.onrender.com/artists"
    ).then((res) => res.json());
  ...
  const fetchHorario = async () => {
    console.log(name);
    const data = await fetch(
      "https://tattoomax-backend-48q8.onrender.com/horarios"
    ).then((res) => res.json());
      ...
      {!isLoading && (
        <>
          ...
          <div className={style.container}>
            ...
            <div>
              <button onClick={openCom} className={style.button}>
                Escribir un comentario
              </button>
              {isComOpen && (
                <AddCom user={userLog} artist={artista} onClose={closeCom} />
              )}
