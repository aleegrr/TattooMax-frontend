...
const Login = () => {
    ...
    async function recibeUserLog(idUser) {
        const datos = await fetch('https://tattoomax-backend-48q8.onrender.com/users').then(res => res.json()).then(data => { userLog = data.find(user => user._id === idUser) });
    ...
    const iniciarSesion = () => {
        ...
        fetch('https://tattoomax-backend-48q8.onrender.com/users/signin', { method: 'POST', cache: 'no-cache', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
            .then(response => {
                if (!response.ok) {
        ...
        if (validateForm()) {
            ...
            fetch('https://tattoomax-backend-48q8.onrender.com/users/signup', {
                ...
                .then(data => {
                    ...
                    sessionStorage.setItem('token', data._id);
                    return fetch('https://tattoomax-backend-48q8.onrender.com/users/signin', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
